...
def main():
    ...
    # create_or_replace_file("parser_result.txt")
    Output_list =[]

    file_list =[]
    log_dir = os.path.join(executable_dir, "Log")
    try:
        file_list = get_file_list(log_dir)
    except FileNotFoundError:
        print(f"錯誤：找不到 Log 目錄: {log_dir}")
        return  # 或做其他錯誤處理
    
    # file_list = get_file_list(join(os.getcwd(),"Log"))
    #print(file_list)

    # print_file_content(join(os.getcwd(),"Log/MyProgram"))
    #list_files_scandir(join(os.getcwd(),"Log"))
    #search_string_in_file(args.filepath, args.search_string)

    # filepath = "Fiona__WY0950700M2N01.txt"  # 替换为您的文件路径
    # test_item = get_value_from_Keyfile()
    # test_item = r"led3%cr%led"
    test_item = get_value_from_Keyfile()
    if not test_item:
        print("錯誤：無法從 Keyfile 取得搜尋字串。")
        return
    ...
    # if isinstance(result, list):  # 检查 result 是否为列表
    #     if result:
    #         print(f"Found pattern '{target_pattern}' in the following lines:")
    #         for line in result:
    #             print(line)
    #     else:
    #         print(f"Pattern '{target_pattern}' not found in the file.")
    # else:  # 如果 result 是字符串，则表示发生了错误
    #     print(result)
